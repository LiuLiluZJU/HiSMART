...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
                performBootstrapping=True,numberOfBootstrapSamples=1000,confidence = 0.95):
    '''
            ...
            if len(nodules.keys()) > maxNumberOfCADMarks:
                ...
                for keytemp, noduletemp in nodules.items():
                    if nrNodules2 >= maxNumberOfCADMarks:
                        break
                    if float(noduletemp.CADprobability) > probThreshold: #choose maxNumberOfCADMarks nodules
    ...
    print("Sensitivity: %.9f\n" % (float(candTPs) / float(totalNumberOfNodules)))
    # compute FROC
    # print(FROCGTList)
    # print (FROCProbList)
    print (len(FROCGTList))
    fps, sens, thresholds = computeFROC(FROCGTList,FROCProbList,len(seriesUIDs),excludeList) #计算FROC值，其实返回的是召回率与假阳性率的列表
    ...
    #     +sens_itp[fps_itp==4]+sens_itp[fps_itp==8]) ##average
    if performBootstrapping:
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        # font = plt.font_manager.FontProperties(fname='msyh.ttf')
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir):
    ...
    #allNodules dict{patient1:[[nodule1],[nodule2],....],patient2:[[nodule1],[nodule2],....],...}
...
if __name__ == '__main__':
    ...
    results_filename = './annotations/All-predanno-1.5.csv'
    # fig1 = plt.figure()
